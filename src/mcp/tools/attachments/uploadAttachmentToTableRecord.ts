...
export function registerUploadAttachmentToTableRecord(server: McpServer) {
  server.tool(
    "upload_attachment_to_table_record",
    "Upload a file to a table record field. Provide file_url (HTTP) or file_content_base64. Uses presigned URL + S3 PUT.",
    {
      organization_id: z.string().describe("Organization ID"),
      table_record_id: z.string().describe("Table record ID"),
    ...
    async (input) => {
      try {
        ...
        await pipefyRequest<any>(M_SET_TABLE_RECORD_FIELD_VALUE, {
          input: { table_record_id: input.table_record_id, field_id: input.field_id, value: [storagePath] },
        });

        return jsonResult({ success: true, table_record_id: input.table_record_id, field_id: input.field_id, file_name: input.file_name, url: storagePath });
      } catch (e) {
