...
export function registerGetFieldCondition(server: McpServer) {
  server.tool("get_field_condition", "Get a field condition and check if it's true for a specific card",
    ...
    async ({ id, cardId }) => {
      try { const data = await pipefyRequest<{ fieldCondition: any }>(Q_FIELD_CONDITION, { id, cardId }); if (!data.fieldCondition) return errorResult("NOT_FOUND", `Field condition ${id} not found`); return jsonResult(data.fieldCondition); }
