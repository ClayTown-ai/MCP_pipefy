...
export function registerGetPlatformAppConfig(server: McpServer) {
  server.tool("get_platform_app_config", "Get a platform app's configuration for a resource",
    ...
    async (input) => {
      try { const data = await pipefyRequest<{ platformAppConfiguration: any }>(Q_PLATFORM_APP_CONFIG, input); if (!data.platformAppConfiguration) return errorResult("NOT_FOUND", `Platform app configuration not found`); return jsonResult(data.platformAppConfiguration); }
